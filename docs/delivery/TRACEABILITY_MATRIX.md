...
| V1-M4-04 | **Done 2026-08-27**; FR-006/009, ADR-004; exact owner-approved GeoNames LK product subset, stable IDs, Colombo default, English-only resources and provenance in [evidence](V1-M4-04_TOWN_CATALOG_EVIDENCE.md); `TownCatalogTest` passes, while M4-05/UI/provider/network assembly remains unauthorized |
| V1-M4-05 | **Done 2026-08-28**; FR-004–009, CR-009/010; application-root repository/provider composition, foreground-only precise/approximate permission path, nine-town offline search, Colombo fallback, recovery/reset and truthful privacy copy; Developer verification, independent QA and Lead acceptance passed; connected execution unavailable because `adb` is absent |

Approved planning task IDs are traceability assignments, not automatic task-start authorization. V1-M1-01 through V1-M3-04 have been explicitly started and are Done. M1, M2, and M3 are explicitly Lead/owner accepted and incorporated into `dev`; accepted M3 history and evidence is recorded through `3d9d9d2`. V1-M4-01 through V1-M4-05 are Done with their acceptance records retained separately. Every later task remains unstarted (or Blocked where recorded) and requires separate explicit authorization. Deferred M9/release evidence remains mandatory under its approved task-specific ownership: V1-M9-01 accessibility/adaptive/localization/manual visual; V1-M9-02 API runtime and complete API/state regression; V1-M9-03 physical-family/OEM/performance/privacy; V1-M9-04 signing/reproducible APK/install-update-rollback/update lineage; V1-M9-05 final QA/family acceptance/release validation.
