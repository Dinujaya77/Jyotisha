...
| Approval state | PA-001, CR-001, PA-002, DA-001, PA-004, DA-004, and DP-001 Approved; M1 through M4 Done, explicitly Lead/owner accepted, and incorporated into `dev`; V1-M4-01 through V1-M4-05 Done; PA-003/CP-003 Blocked |

## Current phase

**Version 1.0 M1 through M4 are Done, explicitly Lead/owner accepted, and incorporated into `dev`.** V1-M4-01 through V1-M4-05 passed Developer verification and final independent QA with zero unresolved Critical, High, or Medium findings. M4 supplies foreground one-shot location acquisition, permission/lifecycle reconciliation, the approved offline nine-town catalogue, versioned mutually exclusive no-backup persistence, and truthful recovery/advisory presentation. The final correction linearized completion eligibility, committed state, and UI publication against cancellation/supersession. Focused M4 JVM tests passed 79/79, the full JVM suite passed 125/125, lint reported zero errors, debug and Android-test APK builds passed, and representative API-36 connected acceptance evidence passed 5/5 on 2026-09-05. No fresh Gradle rerun was available during owner-authorized closure because of the host usage-limit/Gradle-cache environment; acceptance relies on the recorded successful verification evidence. M5 remains unstarted.
...
- Structure: one app module/activity, pure domain engines, immutable state publication, and manual composition; DEP-011–DEP-013 Lifecycle 2.9.2 and DEP-014 DataStore 1.2.1 are declared and verified. `kotlinx-coroutines-core-jvm:1.9.0` remains transitive-only.
...
- Direct dependencies: `lifecycle-viewmodel-ktx`, `lifecycle-viewmodel-compose`, and `lifecycle-runtime-compose`, declared and resolved at 2.9.2 through the existing version catalogue. This historical M1 entry predates M4; DEP-014 DataStore Preferences 1.2.1 was subsequently implemented under V1-M4-03, while unrelated dependencies were not added.
...
- ADR-001–ADR-009 and DEP-011–DEP-014 are approved under PA-002. DEP-011–DEP-013 are implemented under V1-M1-03; DEP-014 is implemented under V1-M4-03 as direct `androidx.datastore:datastore-preferences:1.2.1`, with `kotlinx-coroutines-core-jvm:1.9.0` transitive-only. SOLAR-001 is Approved under PA-004 and remains separately task-gated.
- The exact nine GeoNames product rows, Colombo row/default, hash, and approved coverage are recorded for completed and accepted V1-M4-04 in [its evidence](V1-M4-04_TOWN_CATALOG_EVIDENCE.md); private family-town coverage remains outside this approved subset. DEP-014 resolution/transitives and static installed-path/backup evidence are complete. Complete API/state regression and physical Android provider/OEM/device-transfer behavior remain V1-M9-02/V1-M9-03 obligations. DEP-011–DEP-013 evidence is complete under V1-M1-03. Permanent package identity is complete under V1-M1-02; signed update-lineage evidence remains a release gate.
