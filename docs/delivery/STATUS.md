...
- Town catalogue and location composition: the exact nine owner-approved GeoNames `LK.zip` product rows, Colombo default, hash and review are recorded in [V1-M4-04 Town Catalogue Evidence](V1-M4-04_TOWN_CATALOG_EVIDENCE.md). V1-M4-05 is Done with foreground-only repository/provider composition, offline town search, recovery/reset, and no-backup/device-transfer privacy disclosure; no networking or background location was added. Connected execution remains unavailable because `adb` is absent.
