...
DA-004 and DP-001 became valid only when the repository owner explicitly supplied both phrases. They approved the Version 1.0 UI specification and delivery plan, not automatic implementation; at approval time coding had not started. The owner subsequently started V1-M1-01 through V1-M1-05, which are Done on the unmerged `milestone/M1-android-foundation` branch; M1 remains In Review and M2 or later work has not started. M5 depends on an accepted M1, M6 depends on accepted M5 and CP-001, and M8 remains Blocked by PA-003/CP-003. Fixed Kala Hora and Panchama remain Version 1.1.
