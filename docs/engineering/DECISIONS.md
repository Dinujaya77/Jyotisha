...
| Approval state | CR-001/Option C approved; ADR-001–ADR-009 remain proposed pending architecture reconciliation/approval |
...
- **Decision:** Select a small independently written pure-Kotlin NOAA/Meeus-style method family. Proposed date range `1900-01-01..2100-12-31`; solve centre altitude `-0.8333°`; quantize each anchor once to nearest millisecond; convert to UTC integer nanoseconds; display nearest minute without changing membership.
...
- **Approval record:** Awaiting `APPROVE VERSION 1.0 ARCHITECTURE`.
...
- **Lifecycle:** Explicit Cancel, superseding request, ViewModel clear, or genuine backgrounding cancels. Configuration recreation retains the ViewModel/request. Lifecycle cancellation is silent, retains the prior snapshot, and never leaves a false failure warning or silently restarts acquisition. No Activity reference in the ViewModel; calculation/persistence run off-main.
...
- **Approval record:** Source and schema await architecture approval; exact snapshot/records remain a Phase C prerequisite.
...
- **Decision:** Use Preferences DataStore 1.2.1 for one versioned discriminated active-mode record. Device mode contains only current coordinates, accuracy, permission precision, acquisition instant, zone, and source; manual mode contains only town ID/selection time; default contains dataset provenance. Modes are mutually exclusive. Do not persist derived schedules or history.
...
- **Backup:** Isolate the DataStore in a dedicated location-data directory and exclude the complete directory/domain, including temporary/replacement/companion artifacts, from legacy Auto Backup and Android 12+ cloud/device transfer. Verify installed paths and restore/transfer behavior; disable app backup if exclusion cannot be proven.
- **Dependencies:** Proposed DEP-014 DataStore Preferences.
- **Approval record:** Awaiting `APPROVE VERSION 1.0 ARCHITECTURE`.
...
- **Approval record:** Awaiting architecture approval and device verification before final release support claim.

## ADR-007 — Application structure, state, and dependencies

- **State:** Proposed.
- **Linked requirements:** FR-001–FR-011; NFR-001–NFR-014; proposed FR-012–FR-015 and NFR-015.
...
- **Proposed dependencies:** Lifecycle ViewModel KTX, Lifecycle ViewModel Compose, Lifecycle Runtime Compose at the existing 2.9.2 family, plus Preferences DataStore 1.2.1.
...
- **Extension:** New timing/Panchanga capabilities require explicit system-specific calculators and approved profiles/engines/domain models. A generic `Hora`/`Panchanga` engine must not hide fixed-versus-seasonal behavior or invented lunar/traditional rules.
- **Approval record:** Awaiting `APPROVE VERSION 1.0 ARCHITECTURE`.

## ADR-008 — Timing-system separation and shared calculation context

- **State:** Proposed under Change Request CR-001; architecture approval held.
- **Linked requirements:** FR-012–FR-015; NFR-015; CP-001–CP-003.
- **Decision:** Keep `PlanetaryHoraCalculator`, `KalaHoraCalculator`, `PanchamaKalaCalculator`, and `RahuKalaCalculator` as separate pure boundaries. Feed them one immutable `DailyCalculationContext` containing shared solar events, location, zone, civil date, clock instant, and fingerprint. Publish one atomic `DailyTimingBundle` and select current/next intervals outside UI code.
...
- **Approval record:** Requires Change Request CR-001 approval and later architecture approval; does not authorize production code.

## ADR-009 — Approved-profile execution and fixed-duration coverage gate

- **State:** Proposed under Change Request CR-001; CP-002 subdecision Blocked.
...
- **Approval record:** Requirements-amendment approval records the product need/allocation only; it does not approve this ADR or any profile/rule. ADR-009 requires later architecture approval; CP-002 additionally requires explicit domain/traditional approval, while CP-003/RK requires selected-tradition/golden-vector approval.

## Remaining decisions outside this gate

CP-002 coverage, CP-003 rule evidence, final package/application name, family-device inventory, exact GeoNames snapshot/coordinates and family-town coverage, actual independent astronomical dataset, traditional authority, final UI direction/specification, signing owner/distribution mechanics, and release calculation accuracy remain separately gated. CR-001/Option C is approved but does not approve any ADR.
