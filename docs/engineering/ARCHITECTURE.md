...
| Sri Lankan towns | Exact nine-row product data approved; V1-M4-04 Done and accepted | Frozen GeoNames CC BY 4.0/WGS84 snapshot, schema, stable IDs, offline use, attribution, extraction, exact rows and Colombo default are recorded in [V1-M4-04 Town Catalogue Evidence](../delivery/V1-M4-04_TOWN_CATALOG_EVIDENCE.md). Private family-town coverage remains outside this subset. Town coordinates are source/town-centre points, not physical-accuracy claims. |
...
| Dependencies | DEP-011–DEP-014 implemented and verified | V1-M1-03 added Lifecycle ViewModel KTX, ViewModel Compose, and Runtime Compose 2.9.2. V1-M4-03 declares DataStore Preferences 1.2.1; its resolved transitives and accepted evidence are recorded in the dependency register. `kotlinx-coroutines-core-jvm:1.9.0` is transitive-only. No Navigation, Hilt, Room, solar, Play Services, networking, analytics or encryption dependency. |
...
The Gradle root `Jyotisha` has one Android application module with namespace/application ID `io.github.dinujaya77.jyotisha`. One `MainActivity` hosts a bounded manual Compose shell for Dashboard, Timeline, and Method plus Location, Settings, and About children. Current SDK values are `compileSdk 36`, `targetSdk 36`, and provisional `minSdk 26`; only `minSdk` sets the install floor. AndroidX Core KTX 1.17.0 provides `LocationManagerCompat`; Lifecycle 2.9.2 and DataStore Preferences 1.2.1 are declared. The manifest declares only foreground coarse/fine location permissions, with no background location or network permission, and the no-backup/data-extraction rules exclude the location-data directory. Accepted M4 supplies foreground one-shot acquisition, permission/lifecycle reconciliation, offline town selection, versioned mutually exclusive persistence and truthful recovery presentation. Deterministic location, clock/zone, shell reducer and Compose tests exist. M2 implements the approved 21-role Celestial Archive light/dark semantic palette, fixed Material 3 mapping with dynamic colour disabled, typography, spacing/gutters, shapes, elevation, wash, motion/reduced-motion, icon/illustration, focus and 48dp interaction-target tokens, accessible reusable presentation components, and eight deterministic synthetic preview fixtures. Solar, Hora, Rahu, and live calculation-state integration remain unimplemented.
...
M1 through M4 are Done, explicitly Lead/owner accepted, and incorporated into `dev`. The shell uses primitive saveable state for activity/configuration recreation; full operating-system process-death restoration is not claimed. Production solar/Hora states remain unavailable and synthetic calculation data remains preview/test-only. M5 and later milestones remain unstarted (or Blocked where recorded). Manual TalkBack, D-pad/focus-restoration, actual 200% font-scale and compact-landscape inspection, grayscale/high-contrast/OLED review, magnification, API-26 runtime, physical-family-device, screenshot, signing, update-lineage and release evidence remain mandatory under their approved task-specific gates.
