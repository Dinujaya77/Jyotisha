...
| Dependencies | DEP-011–DEP-014 implemented; DEP-014 corrective QA pending | V1-M1-03 added Lifecycle ViewModel KTX, ViewModel Compose, and Runtime Compose 2.9.2. V1-M4-03 declares DataStore Preferences 1.2.1; its resolved transitives and corrective evidence are recorded in the dependency register. No Navigation, Hilt, Room, solar, Play Services, networking, analytics or encryption dependency. Direct coroutine APIs may not rely silently on transitives. |
...
M1, M2, and M3 are Done, explicitly Lead/owner accepted, and incorporated into `dev`. The shell uses primitive saveable state for activity/configuration recreation; full operating-system process-death restoration is not claimed. V1-M3-01 through V1-M3-04 preserve the bounded shell. Production solar/Hora states remain unavailable and synthetic calculation data remains preview/test-only. M4 is In Progress in corrective QA; M5 and later milestones remain unstarted (or Blocked where recorded). Manual TalkBack, D-pad/focus-restoration, actual 200% font-scale and compact-landscape inspection, grayscale/high-contrast/OLED review, magnification, API-26 runtime, physical-family-device, screenshot, signing, update-lineage and release evidence remain mandatory under their approved task-specific gates.
