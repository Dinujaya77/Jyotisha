...
DA-004 approves this specification after documentation QA; evidence is produced according to the approved delivery tasks. M2 acceptance requires the automated design-system evidence assigned to V1-M2-01 through V1-M2-04: semantic-colour verification and contrast calculations, token/component tests, Compose source and preview compilation, automated accessibility semantics available within current tooling, deterministic preview fixtures, and the recorded API-36 automated/connected evidence. M2 and M3 acceptance do not require final closure of the manual or release matrix, although every task must still satisfy its own listed acceptance tests. V1-M9-01 closes manual TalkBack, keyboard/D-pad/focus restoration, actual 200% font-scale, compact-landscape/adaptive, grayscale/high-contrast/OLED and pressed/focus/disabled visual inspection, magnification, localization/expansion, non-colour recognition, and final screenshot/manual visual evidence; V1-M9-02 closes API-26 runtime and the complete API/state regression matrix; V1-M9-03 closes oldest/newest physical-family-device and OEM performance/privacy evidence; V1-M9-04 closes signing and reproducible APK install/update/rollback/update-lineage evidence; and V1-M9-05 performs final independent QA, owner family acceptance, and release validation. These final requirements are deferred, not removed; record produced evidence against the UI IDs in `UI_SPEC.md`, `ACCESSIBILITY.md`, and their assigned delivery tasks.
